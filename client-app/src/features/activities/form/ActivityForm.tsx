...
const  validate = combineValidators({
    ...
    description:composeValidators(
        isRequired('Description'), hasLengthGreaterThan(4)({message:'Description needs to be at least 5 characters'})
    )('Description'),
...
export const ActivityForm:React.FC<RouteComponentProps<DetailParams>> = ({match, history}) => {
    ...
    const handleFinalFormSubmit = (values:any)=> {
        const dateAntTime = combineDatesAndTime(values.date, values.time);
        const {date, time, ...activity} = values;
        activity.date = dateAntTime;
        if (!activity.id){
        ...
        <Grid>
            ...
            <Segment clearing>
                <FinalForm validate={validate} initialValues={activity} onSubmit={handleFinalFormSubmit} render={({handleSubmit, invalid, pristine}) =>(
                    ...
                    <Field name='description' placeholder='Description' value={activity.description} component={TextAreaInput}/>
                    ...
                    <Button onClick={activity.id ? () => history.push(`/activity/${activity.id}`) : () => history.push('/activities')}
                    floated='right' positive type='button' content='Cancel' loading = {submitting} disabled={loading}/>                 
                </Form>
                )}/>            
        </Segment>
            </Grid.Column>
